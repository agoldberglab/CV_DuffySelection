...
runSLiM <- function(seed, s, m, N, h, gen, rate, cont_adm, map, model_name, slim_model)
{
...
option_list <- list(
  ...
  make_option(c("-t", "--gen"), type="character", default=20,
...
model_name <- paste0(opt$out, "gen-", gen, model_growth, "_", model_adm)
...
mapply(seed=seed_vector, FUN = runSLiM, s=s, map=map, m=m, gen=gen, N=N, h=h, rate=rate,
