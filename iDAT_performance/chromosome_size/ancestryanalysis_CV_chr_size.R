...
coords_file <- commandArgs(trailingOnly=TRUE)[2]
chrom_length <- as.numeric(commandArgs(trailingOnly=TRUE)[3])
bene_locus <- as.integer(chrom_length * 0.8)


#get seed number and selection coeff
pattern <- "^.+/.+L-(\\d+).+_m-(\\d.*\\d*)_s-(\\d.*\\d*)_seed-(\\d+)_"
...
cat(seed, s, chrom_length, m, bene_rank, bene_prop, mean_globalancestry, var_prop, var_prop_10Mb,
